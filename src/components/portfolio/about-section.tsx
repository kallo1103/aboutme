...
export function AboutSection({ about, skills }: AboutSectionProps) {
  return (
    ...
    >
      ...
      {/* Layer 4: Content */}
      <div className="liquidGlass-text flex flex-col gap-8 sm:gap-10 lg:gap-12">
        <div className="grid gap-6 sm:gap-8 md:gap-10 lg:grid-cols-[1.4fr,1fr] lg:items-start lg:gap-12">
        ...
        >
          {about.focusAreas.map((focus, index) => (
            <motion.div
              ...
              className="flex h-full flex-col rounded-xl p-4 border border-white/40 bg-white/30 shadow-sm backdrop-blur-sm sm:rounded-2xl sm:p-5 md:p-6"
            >
              <h3 className="text-base font-semibold text-zinc-900 sm:text-lg">{focus.title}</h3>
              <p className="mt-1.5 flex-1 text-xs text-zinc-600 sm:mt-2 sm:text-sm">{focus.description}</p>
            </motion.div>
          ))}
        </motion.div>
      </div>
